...
let tunerA4Freq = 440;
let tunerSelectedInstrument = 'chromatic';

// All 12 note names in chromatic scale
const TUNER_NOTES = ['C', 'C#', 'D', 'D#', 'E', 'F', 'F#', 'G', 'G#', 'A', 'A#', 'B'];

// Instrument tuning references (standard tuning, in Hz)
const TUNER_INSTRUMENTS = {
  chromatic: { name: 'Chromatic (All Notes)', notes: null },
  flute: { name: 'Flute', notes: ['C4','D4','E4','F4','G4','A4','B4','C5','D5','E5','F5','G5','A5','B5','C6','D6','E6','F6','G6','A6','B6','C7'] },
  clarinet: { name: 'Clarinet (Bb)', notes: ['E3','F3','G3','A3','Bb3','C4','D4','E4','F4','G4','A4','Bb4','C5','D5','E5','F5','G5','A5','Bb5','C6'] },
  trumpet: { name: 'Trumpet (Bb)', notes: ['F#3','G3','A3','Bb3','C4','D4','E4','F4','G4','A4','Bb4','C5','D5','E5','F5','G5','A5','Bb5','C6'] },
  alto_sax: { name: 'Alto Saxophone (Eb)', notes: ['Db3','Eb3','F3','G3','Ab3','Bb3','C4','Db4','Eb4','F4','G4','Ab4','Bb4','C5','Db5','Eb5','F5'] },
  tenor_sax: { name: 'Tenor Saxophone (Bb)', notes: ['Ab2','Bb2','C3','D3','Eb3','F3','G3','Ab3','Bb3','C4','D4','Eb4','F4','G4','Ab4','Bb4','C5'] },
  trombone: { name: 'Trombone', notes: ['E2','F2','G2','A2','Bb2','C3','D3','E3','F3','G3','A3','Bb3','C4','D4','E4','F4','G4','A4','Bb4'] },
  french_horn: { name: 'French Horn (F)', notes: ['B2','C3','D3','E3','F3','G3','A3','Bb3','C4','D4','E4','F4','G4','A4','Bb4','C5','D5','E5','F5'] },
  tuba: { name: 'Tuba', notes: ['Bb1','C2','D2','Eb2','F2','G2','A2','Bb2','C3','D3','Eb3','F3','G3','A3','Bb3'] },
  violin: { name: 'Violin', notes: ['G3','D4','A4','E5'] },
  viola: { name: 'Viola', notes: ['C3','G3','D4','A4'] },
  cello: { name: 'Cello', notes: ['C2','G2','D3','A3'] },
  bass: { name: 'String Bass', notes: ['E1','A1','D2','G2'] },
  guitar: { name: 'Guitar', notes: ['E2','A2','D3','G3','B3','E4'] },
  oboe: { name: 'Oboe', notes: ['Bb3','C4','D4','E4','F4','G4','A4','Bb4','C5','D5','E5','F5','G5','A5','Bb5','C6'] },
  bassoon: { name: 'Bassoon', notes: ['Bb1','C2','D2','E2','F2','G2','A2','Bb2','C3','D3','E3','F3','G3','A3','Bb3','C4','D4','E4'] },
};

function tunerInit() {
  // Populate instrument select
  var select = document.getElementById('tuner-instrument');
  if (!select || select.options.length > 1) return;
  select.innerHTML = '';
  Object.keys(TUNER_INSTRUMENTS).forEach(function(key) {
    var opt = document.createElement('option');
    opt.value = key;
    opt.textContent = TUNER_INSTRUMENTS[key].name;
    select.appendChild(opt);
  });
}

function tunerSetInstrument(key) {
  tunerSelectedInstrument = key;
  // Show open strings / reference notes for string instruments
  var refEl = document.getElementById('tuner-ref-notes');
  if (!refEl) return;
  var inst = TUNER_INSTRUMENTS[key];
  if (!inst || !inst.notes || key === 'chromatic') {
    refEl.innerHTML = '';
    return;
  }
  if (['violin','viola','cello','bass','guitar'].indexOf(key) !== -1) {
    refEl.innerHTML = '<div class="tuner-ref-label">Open Strings</div><div class="tuner-ref-list">' +
      inst.notes.map(function(n) { return '<span class="tuner-ref-note">' + n + '</span>'; }).join('') + '</div>';
  } else {
    refEl.innerHTML = '';
  }
}

function tunerSetA4(val) {
  tunerA4Freq = parseInt(val) || 440;
  document.getElementById('tuner-a4-display').textContent = tunerA4Freq + ' Hz';
}

async function tunerStart() {
  if (tunerIsRunning) return;
  ...
  try {
    ...
    tunerIsRunning = true;
    document.getElementById('tuner-startBtn').disabled = true;
    document.getElementById('tuner-stopBtn').disabled = false;
    document.getElementById('tuner-status').textContent = 'Listening...';
    document.getElementById('tuner-status').style.color = 'var(--accent-green)';

    tunerLoop();
  } catch(e) {
    document.getElementById('tuner-status').textContent = 'Microphone access denied. Please allow microphone access to use the tuner.';
    document.getElementById('tuner-status').style.color = 'var(--accent-red)';
  }
}
...
function tunerStop() {
  ...
  document.getElementById('tuner-startBtn').disabled = false;
  document.getElementById('tuner-stopBtn').disabled = true;
  document.getElementById('tuner-status').textContent = 'Stopped';
  document.getElementById('tuner-status').style.color = 'var(--text-muted)';
  document.getElementById('tuner-note-display').textContent = '—';
  document.getElementById('tuner-freq-display').textContent = '— Hz';
...
function tunerLoop() {
  ...
  if (rms < 0.008) {
    document.getElementById('tuner-note-display').textContent = '—';
    document.getElementById('tuner-freq-display').textContent = '— Hz';
    document.getElementById('tuner-cents-display').textContent = 'Play a note...';
    document.getElementById('tuner-needle').style.transform = 'rotate(0deg)';
    document.getElementById('tuner-indicator').className = 'tuner-indicator';
    return;
  }

  var freq = tunerAutoCorrelate(buffer, tunerAudioCtx.sampleRate);
  if (freq === -1) return;

  // Find closest note
  var noteInfo = tunerFreqToNote(freq);

  document.getElementById('tuner-note-display').textContent = noteInfo.name;
  document.getElementById('tuner-freq-display').textContent = freq.toFixed(1) + ' Hz';

  var centsAbs = Math.abs(noteInfo.cents);
  var centsSign = noteInfo.cents >= 0 ? '+' : '';
  document.getElementById('tuner-cents-display').textContent = centsSign + noteInfo.cents.toFixed(0) + ' cents';

  // Needle rotation: -50 to +50 cents maps to -45deg to +45deg
  var needleAngle = Math.max(-45, Math.min(45, noteInfo.cents * 0.9));
  document.getElementById('tuner-needle').style.transform = 'rotate(' + needleAngle + 'deg)';

  // Color indicator
  var indicator = document.getElementById('tuner-indicator');
  if (centsAbs <= 5) {
    indicator.className = 'tuner-indicator tuner-in-tune';
  } else if (centsAbs <= 15) {
    indicator.className = 'tuner-indicator tuner-close';
  } else {
    indicator.className = 'tuner-indicator tuner-out';
  }

  // Octave display
  document.getElementById('tuner-octave-display').textContent = noteInfo.octave;
}
...
function tunerAutoCorrelate(buf, sampleRate) {
  ...
  for (var offset = 1; offset < maxSamples; offset++) {
    var correlation = 0;
    ...
    if (correlation > 0.9 && correlation > bestCorrelation) {
      ...
      // Found a peak, now we can interpolate
...
function tunerFreqToNote(freq) {
  // How many semitones from A4
  var semitones = 12 * Math.log2(freq / tunerA4Freq);
  var roundedSemitones = Math.round(semitones);
  var cents = (semitones - roundedSemitones) * 100;

  // Note index (A = 9 in our TUNER_NOTES array starting at C)
  var noteIdx = ((roundedSemitones % 12) + 12 + 9) % 12;
  var octave = 4 + Math.floor((roundedSemitones + 9) / 12);

  // Adjust for Bb notation (prefer flats for Bb instruments)
  var noteName = TUNER_NOTES[noteIdx];
...
function tunerPlayReference(freq) {
  var ctx = new (window.AudioContext || window.webkitAudioContext)();
  var osc = ctx.createOscillator();
  var gain = ctx.createGain();
  osc.connect(gain);
  gain.connect(ctx.destination);
  osc.frequency.value = freq || tunerA4Freq;
